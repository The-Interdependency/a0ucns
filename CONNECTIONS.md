...
|---|---|---|---|---|
| `a0-betatest/` | The-Interdependency/a0-betatest | `8647a4950b8f37d7aa2e1bb6aa44e3c7835f89fd` | 2026-07-09 | `storage/agents/` (per-agent ZFAE `zfae_core.safetensors` checkpoints — non-committable per a0-betatest doctrine), `_legacy_a0/uploads/` (~142M upload blobs). Both replaced by a short exclusion-marker file. Re-pinned to merged `main` after the Coupling-C + training work landed — carries the Odysseus connector (#9: `tools/odysseus_relay.py` scoped `/api/codex/*` catalogue tools, provider-safe tool naming), the Chat Training tab + Agent Creation Lab (#10: `interdependent_lib/ucns_embed.py` UCNS-native embedding, `edcm_readout.py`, `gonal_stack.py` cylindrical disk-stack, `api_training.py`, `api_agent_lab.py` permutation composer, plus the `ChatTrainingPage`/`AgentLabPage` frontend), the MCP registry-hydration fix (#11), and the deploy import-smoke + completed `requirements.txt` (#12), on top of the earlier security-triage fixes (#4–#8). |
