...
|---|---|---|---|---|
| `a0-betatest/` | The-Interdependency/a0-betatest | `de3464b5227d7ca50493f12f0d5eb6d9ff7d20ed` | 2026-07-08 | `storage/agents/` (per-agent ZFAE `zfae_core.safetensors` checkpoints — non-committable per a0-betatest doctrine), `_legacy_a0/uploads/` (~142M upload blobs). Both replaced by a short exclusion-marker file. Re-pinned to merged `main` after the Coupling-C + training work landed — now also carries the Odysseus connector (#9: `tools/odysseus_relay.py` scoped `/api/codex/*` catalogue tools, provider-safe tool naming) and the Chat Training tab + Agent Creation Lab (#10: `interdependent_lib/ucns_embed.py` UCNS-native embedding, `edcm_readout.py`, `gonal_stack.py` cylindrical disk-stack, `api_training.py`, `api_agent_lab.py` permutation composer, plus the `ChatTrainingPage`/`AgentLabPage` frontend), on top of the earlier security-triage fixes (#4–#8). |
