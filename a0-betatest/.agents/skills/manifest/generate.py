...
Living-spec tool (msdmd family). Vendored from The-Interdependency/skill-lib.
See SKILL.md alongside for the full convention.

Usage (exactly one mode is required):
  python generate.py --print
  python generate.py --write
...
def render(facts: dict, refresh_cmd: str) -> str:
    note = f"<!-- AUTO-GENERATED by .agents/skills/manifest/generate.py. Refresh: `{refresh_cmd}`. Do not edit between markers. -->"
    ...
    lines += [
        "",
        f"Derived from `{facts['pyproject']}` + the repo tree. Unknown fields surface as `{HMMM}` rather than a guess.",
        END,
    ]
    return "\n".join(lines)
